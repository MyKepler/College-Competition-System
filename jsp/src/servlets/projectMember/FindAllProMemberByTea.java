...
/**
 * Servlet implementation class FindAllProMemberByTea
 */
@WebServlet("/FindAllProMemberByTea")
public class FindAllProMemberByTea extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FindAllProMemberByTea() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ProjectMember findAllProMemberByTeaDao = new ProjectMemberImpl();
		//输出流
		...
		if(StringUtils.isNotBlank(pageNum) && StringUtils.isNotBlank(pageSize) && StringUtils.isNotBlank(userId)) {
			proMemberList = findAllProMemberByTeaDao.findAllProInfoByTea(pageNum, pageSize, userId);
