...
/**
 * Servlet implementation class FindAllProRevByTea
 */
@WebServlet("/FindAllProRevByTea")
public class FindAllProRevByTea extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FindAllProRevByTea() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//学生对象
		ProReview findAllProByTeaDao = new ProReviewImpl();
		//输出流
		...
		if(StringUtils.isNotBlank(pageNum) && StringUtils.isNotBlank(pageSize) && StringUtils.isNotBlank(userId)) {
			proList = findAllProByTeaDao.findProReviewByTea(pageNum, pageSize, userId);
			if(proList != null) {
