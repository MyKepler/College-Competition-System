...
@WebServlet("/StudentSignUpFileUpload")
public class StudentSignUpFileUpload extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //文件上传的保存地址
        String fileDir = getServletContext().getRealPath("/SignUpFileUpload");
            ...
            while (iter.hasNext()) {
                ...
                if (!formitem.isFormField()) { // 忽略不是上传文件的表单域
                    ...
                    try {
                        ...
                        filepath="SignUpFileUpload/" + filename;
                        //保存到数据库
                        System.out.println("数据库id"+id);
                        i=studentDao.StudentSignUpUploadFiles(id,filepath,exname);
