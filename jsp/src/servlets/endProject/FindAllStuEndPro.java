...
@WebServlet("/FindAllStuEndPro")
public class FindAllStuEndPro extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FindAllStuEndPro() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//学生对象
		EndProject findAllProDao = new EndProjectImpl();
		//输出流
		...
		if(StringUtils.isNotBlank(pageNum) && StringUtils.isNotBlank(pageSize)) {
			proList = findAllProDao.findAllStuEndPro(pageNum,pageSize,userId);
			if(proList != null) {
