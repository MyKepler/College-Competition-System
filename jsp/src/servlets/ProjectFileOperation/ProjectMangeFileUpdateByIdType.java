...
//更新特定id所有上传文件或者单个文件的type（path为*则为全部文件，path为可用地址则为单个文件）
@WebServlet("/ProjectMangeFileUpdateByIdType")
public class ProjectMangeFileUpdateByIdType extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (StringUtils.isNotBlank(id) && StringUtils.isNotBlank(path) && StringUtils.isNotBlank(type)){
            Integer msg = ProjectDao.ProjectSignUpUpdateAllFilesType(id,type,path);
            if(msg<0){
