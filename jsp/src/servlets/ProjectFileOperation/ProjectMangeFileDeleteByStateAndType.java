...
//项目管理中特点id、state和type删除全部文件
@WebServlet("/ProjectMangeFileDeleteByStateAndType")
public class ProjectMangeFileDeleteByStateAndType extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (StringUtils.isNotBlank(id) && StringUtils.isNotBlank(type) && StringUtils.isNotBlank(state)){
            ...
            Integer msg = ProjectDao.ProjectSignUpDeleteByStateAndTypeFiles(id,state,type);
            if(msg<0){
