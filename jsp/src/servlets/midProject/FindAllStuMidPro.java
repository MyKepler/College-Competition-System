...
@WebServlet("/FindAllStuMidPro")
public class FindAllStuMidPro extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FindAllStuMidPro() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//学生对象
		MidProject findAllProDao = new MidProjectImpl();
		//输出流
		...
		if(StringUtils.isNotBlank(pageNum) && StringUtils.isNotBlank(pageSize)) {
			proList = findAllProDao.findAllStuMidPro(pageNum,pageSize,userId);
			if(proList != null) {
