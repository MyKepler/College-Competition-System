<template>
  <div class="container">
    <h2 class="title">项目结题管理</h2>
    <div class="head-info-wrapper">
      <div>
        <span>项目管理 > </span>
        <span v-if= "type==='0'">项目结题查看管理</span>
        <span v-if= "type==='1'">项目结题修改管理</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    toUpdate(){
      this.type = '1';
      this.isCheck = this.type == '0'?true:false
      this.$router.push(`/EndManageInfo/1/${this.$route.params.proId}`);
    },
    // 取消修改
    cancel(){
      this.initEndProView();
    ...
    affirm(){
      ...
      axios.post('/api/jsp/UpdateEndPro',qs.stringify(params))
      .then(response=>{
        console.log(response);
        if(response.data.state=='200'){
          this.$router.push(`/admin/ProjectEndManage`);
        }
      })
      .catch(err=>{
        console.log(err);
    ...
    initEndProView(){
        ...
        axios.post('/api/jsp/FindSpeEndPro',qs.stringify(params))
        .then(response=>{
    ...
    // 下载所有文件
    downloadAll(){  
       window.open("http://localhost:8080/jsp/ProjectMangeFileDownload?type=结题&id="+this.proId);
    },
    // 下载单个文件
    downloadOne(){},
    // 删除全部文件
