...
export default {
  ...
  methods: {
    ...
    toUpdate(){
      ...
      this.$router.push(`/StudentProjectManage/1/${this.proId}`);
    },
    //取消修改
    cancel(){
      this.initProPersonal();
    ...
    affirm(){
      ...
      .then(response=>{
        if(response.data.state=='200'){
          this.$router.push(`/student/StudentProjectInfoManage`);
        }
      })
      .catch(err=>{
        console.log(err);
    ...
    initProPersonal(){
      ...
      .then(response=>{
        ...
        this.teacher.userId =  teaObj.user_id;
        this.teacher.name = teaObj.user_name;
        this.teacher.email = teaObj.user_mail;
        this.teacher.userPhone = teaObj.user_phone;
    ...
    toTeaDetail(){
     ...
     // 下载所有文件
    downloadAll(){
      window.open("http://localhost:8080/jsp/ProjectMangeFileDownload?type=all&id="+this.proId);
    },
    // 下载单个文件
    downloadOne(){},
  }
